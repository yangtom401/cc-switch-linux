...
pub fn get_grok_config_dir() -> PathBuf {
    crate::settings::get_grok_override_dir().unwrap_or_else(|| {
        get_home_dir()
...
/// Validate provider-owned Grok Build settings (JSON wrapper around config.toml).
///
/// 官方条目（category == "official"）不要求自定义模型表，语法合法即可；
/// 非官方供应商必须携带完整的自定义模型配置。
pub fn validate_grok_provider_settings(settings: &Value) -> Result<(), AppError> {
    ...
    match config {
        Some(config) => validate_config_toml_syntax(config),
